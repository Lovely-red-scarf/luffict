...
class CourseDetail(models.Model):
    ...
    prerequisite = models.TextField(verbose_name="课程先修要求", max_length=1024)
    recommend_courses = models.ManyToManyField("Course", related_name="recommend_by", blank=True)   # releted_name 用于反向查询 可以直接使用别名 来进行查询 不需要使用表名和这个字段名直接用别名
    teachers = models.ManyToManyField("Teacher", verbose_name="课程讲师")
...
class CourseChapter(models.Model):
    """课程章节"""
    course = models.ForeignKey("Course",related_name='xxxxx') #  反向查询必须要用这个别名来查 查询你表中的内容必须用别名
